...
import { resolveCli, runCli, parseCached, normalizeCachedJson } from './cli.js'
...
export function renderServed(value) {
  ...
  for (const m of models) {
    // Show the memory-fitted ceiling when the server reports one and it is
    // tighter than the native window — that is the number that actually
    // bounds a session on this machine.
    const ctxTag =
      m.maxModelLen && m.contextWindow && m.maxModelLen < m.contextWindow
        ? `${m.maxModelLen.toLocaleString()} ctx (memory-fit of ${m.contextWindow.toLocaleString()})`
        : m.contextWindow
...
export function registerManagement(ctx, deps) {
  ...
  jsonTool(
    ...
    {},
    async (_args, exec) => {
      const sp = exec.subprocess ?? deps.subprocess
      const executable = await resolveCli(sp, cliCommand)
      // Prefer structured `--json` (rapid-mlx new enough to emit it); fall
      // back to scraping the text table for older servers, where `--json` is
      // an unknown flag and argparse exits non-zero.
      const jsonRun = await runCli(sp, executable, ['models', '--cached', '--json'], {
        signal: exec.signal,
        timeoutMs: 30_000,
      })
      if (jsonRun.code === 0) {
        try {
          return normalizeCachedJson(JSON.parse(jsonRun.stdout))
        } catch {
          // Malformed JSON — fall through to the text parser.
        }
      }
      const { stdout } = await runCli(sp, executable, ['models', '--cached'], {
        signal: exec.signal,
        timeoutMs: 30_000,
      })
