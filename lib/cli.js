...
/** Human age like `22h ago` from a seconds delta, or '' if unknown. */
...
/**
 * Normalize `rapid-mlx models --cached --json` output into the SAME internal
 * shape `parseCached` produces, so the renderer is agnostic to which source
 * the tool read. Preferred over the text scrape when the server is new enough
 * to emit `--json`.
 */
export function normalizeCachedJson(payload) {
  const rows = Array.isArray(payload?.cached) ? payload.cached : []
  const models = rows.map((m) => ({
    alias: typeof m.alias === 'string' ? m.alias : null,
    state: typeof m.state === 'string' ? m.state : 'ok',
    repo: String(m.repo ?? ''),
    sizeText: undefined, // JSON carries bytes; the renderer formats them
    sizeBytes: Number.isFinite(m.size_bytes) ? m.size_bytes : undefined,
    modified: formatAge(m.age_seconds),
  }))
  return {
    models,
    count: Number.isFinite(payload?.count) ? payload.count : models.length,
    totalBytes: Number.isFinite(payload?.total_bytes)
      ? payload.total_bytes
      : models.reduce((sum, m) => sum + (m.sizeBytes ?? 0), 0),
  }
}
